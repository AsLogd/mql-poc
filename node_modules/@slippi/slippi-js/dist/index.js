...
function toHalfwidth(str) {
    ...
    const ret = ___default.map(str, (char) => convertChar(char.charCodeAt(0)));
...
function iterateEvents(slpFile, callback, startPos = null) {
    ...
    const commandPayloadBuffers = ___default.mapValues(slpFile.messageSizes, (size) => new Uint8Array(size + 1));
    ...
    switch (command) {
        case exports.Command.GAME_START:
            const getPlayerObject = (playerIndex) => {
                ...
                const nametag = toHalfwidth(iconv
...
class SlpParser extends events.EventEmitter {
    ...
    getLatestFrame() {
        ...
        const frameIndex = this.latestFrameIndex || exports.Frames.FIRST;
    ...
    _handlePostFrameUpdate(payload) {
        ...
        if (payload.frame <= exports.Frames.FIRST) {
            ...
            const playersByIndex = ___default.keyBy(this.settings.players, "playerIndex");
    ...
    _handleItemUpdate(payload) {
        const items = ___default.get(this.frames, [payload.frame, "items"], []);
        items.push(payload);
...
function calcDamageTaken(frame, prevFrame) {
    const percent = ___default.get(frame, "percent", 0);
    const prevPercent = ___default.get(prevFrame, "percent", 0);
...
function handleActionCompute(state, indices, frame) {
    ...
    const didDashDance = ___default.isEqual(last3Frames, dashDanceAnimations);
...
function handleActionWavedash(counts, animations) {
    const currentAnimation = ___default.last(animations);
    ...
    const recentAnimations = ___default.keyBy(recentFrames, (animation) => animation);
...
class ConversionComputer {
    ...
    _populateConversionTypes() {
        ...
        const conversionsToHandle = ___default.filter(this.conversions, (conversion) => {
            return conversion.openingType === "unknown";
        });
        // Group new conversions by startTime and sort
        const sortedConversions = ___default.chain(conversionsToHandle)
            .groupBy("startFrame")
            .orderBy((conversions) => ___default.get(conversions, [0, "startFrame"]))
            .value();
...
function handleConversionCompute(frames, state, indices, frame, conversions) {
    ...
    const prevPlayerFrame = ___default.get(frames, [playerFrame.frame - 1, "players", indices.playerIndex, "post"], {});
    const opponentFrame = frame.players[indices.opponentIndex].post;
    // FIXME: use type PostFrameUpdateType instead of any
    // This is because the default value {} should not be casted as a type of PostFrameUpdateType
    const prevOpponentFrame = ___default.get(frames, [playerFrame.frame - 1, "players", indices.opponentIndex, "post"], {});
...
function handleInputCompute(frames, state, indices, frame) {
    ...
    const prevPlayerFrame = ___default.get(frames, [
        playerFrame.frame - 1,
        "players",
        indices.playerIndex,
        "pre",
    ]);
    if (playerFrame.frame < exports.Frames.FIRST_PLAYABLE || !prevPlayerFrame) {
...
class Stats {
    ...
    process() {
        ...
        let i = this.lastProcessedFrame ? this.lastProcessedFrame + 1 : exports.Frames.FIRST;
        while (this.frames[i]) {
...
function isCompletedFrame(playerPermutations, frame) {
    ...
    // for our purposes.
    const indices = ___default.first(playerPermutations);
    const playerPostFrame = ___default.get(frame, ["players", indices.playerIndex, "post"]);
    const oppPostFrame = ___default.get(frame, ["players", indices.opponentIndex, "post"]);
...
function generateOverallStats(playerIndices, inputs, stocks, conversions, playableFrameCount) {
    const inputsByPlayer = ___default.keyBy(inputs, "playerIndex");
    const stocksByPlayer = ___default.groupBy(stocks, "playerIndex");
    const conversionsByPlayer = ___default.groupBy(conversions, "playerIndex");
    const conversionsByPlayerByOpening = ___default.mapValues(conversionsByPlayer, (conversions) => ___default.groupBy(conversions, "openingType"));
    ...
    const overall = playerIndices.map((indices) => {
        ...
        const playerInputs = ___default.get(inputsByPlayer, playerIndex) || {};
        const inputCounts = {
            buttons: ___default.get(playerInputs, "buttonInputCount"),
            triggers: ___default.get(playerInputs, "triggerInputCount"),
            cstick: ___default.get(playerInputs, "cstickInputCount"),
            joystick: ___default.get(playerInputs, "joystickInputCount"),
            total: ___default.get(playerInputs, "inputCount"),
        };
        const conversions = ___default.get(conversionsByPlayer, playerIndex) || [];
        const successfulConversions = conversions.filter((conversion) => conversion.moves.length > 1);
        const opponentStocks = ___default.get(stocksByPlayer, opponentIndex) || [];
        const opponentEndedStocks = ___default.filter(opponentStocks, "endFrame");
        const conversionCount = conversions.length;
        const successfulConversionCount = successfulConversions.length;
        const totalDamage = ___default.sumBy(opponentStocks, "currentPercent") || 0;
...
function getOpeningRatio(conversionsByPlayerByOpening, playerIndex, opponentIndex, type) {
    const openings = ___default.get(conversionsByPlayerByOpening, [playerIndex, type]) || [];
    const opponentOpenings = ___default.get(conversionsByPlayerByOpening, [opponentIndex, type]) || [];
...
function getBeneficialTradeRatio(conversionsByPlayerByOpening, playerIndex, opponentIndex) {
    const playerTrades = ___default.get(conversionsByPlayerByOpening, [playerIndex, "trade"]) || [];
    const opponentTrades = ___default.get(conversionsByPlayerByOpening, [opponentIndex, "trade"]) || [];
    const benefitsPlayer = [];
    // Figure out which punishes benefited this player
    const zippedTrades = ___default.zip(playerTrades, opponentTrades);
    zippedTrades.forEach((conversionPair) => {
        const playerConversion = ___default.first(conversionPair);
        const opponentConversion = ___default.last(conversionPair);
...
const externalCharacters = [
    ...
    {
        id: exports.Character.MEWTWO,
        name: "Mewtwo",
        shortName: "Mewtwo",
        colors: ["Default", "Red", "Blue", "Green"],
    },
    {
        id: exports.Character.NESS,
        name: "Ness",
        shortName: "Ness",
    ...
    {
        id: exports.Character.ROY,
        name: "Roy",
        shortName: "Roy",
        colors: ["Default", "Red", "Blue", "Green", "Yellow"],
    },
    {
        id: exports.Character.PICHU,
        name: "Pichu",
        shortName: "Pichu",
        colors: ["Default", "Red", "Blue", "Green"],
    },
...
const stages = {
    [exports.Stage.FOUNTAIN_OF_DREAMS]: {
        id: exports.Stage.FOUNTAIN_OF_DREAMS,
        name: "Fountain of Dreams",
    },
    [exports.Stage.POKEMON_STADIUM]: {
        id: exports.Stage.POKEMON_STADIUM,
        name: "Pokémon Stadium",
    },
    [exports.Stage.PEACHS_CASTLE]: {
        id: exports.Stage.PEACHS_CASTLE,
        name: "Princess Peach's Castle",
    },
    [exports.Stage.KONGO_JUNGLE]: {
        id: exports.Stage.KONGO_JUNGLE,
        name: "Kongo Jungle",
    },
    [exports.Stage.BRINSTAR]: {
        id: exports.Stage.BRINSTAR,
        name: "Brinstar",
    },
    [exports.Stage.CORNERIA]: {
        id: exports.Stage.CORNERIA,
        name: "Corneria",
    },
    [exports.Stage.YOSHIS_STORY]: {
        id: exports.Stage.YOSHIS_STORY,
        name: "Yoshi's Story",
    },
    [exports.Stage.ONETT]: {
        id: exports.Stage.ONETT,
        name: "Onett",
    },
    [exports.Stage.MUTE_CITY]: {
        id: exports.Stage.MUTE_CITY,
        name: "Mute City",
    },
    [exports.Stage.RAINBOW_CRUISE]: {
        id: exports.Stage.RAINBOW_CRUISE,
        name: "Rainbow Cruise",
    },
    [exports.Stage.JUNGLE_JAPES]: {
        id: exports.Stage.JUNGLE_JAPES,
        name: "Jungle Japes",
    },
    [exports.Stage.GREAT_BAY]: {
        id: exports.Stage.GREAT_BAY,
        name: "Great Bay",
    },
    [exports.Stage.HYRULE_TEMPLE]: {
        id: exports.Stage.HYRULE_TEMPLE,
        name: "Hyrule Temple",
    },
    [exports.Stage.BRINSTAR_DEPTHS]: {
        id: exports.Stage.BRINSTAR_DEPTHS,
        name: "Brinstar Depths",
    },
    [exports.Stage.YOSHIS_ISLAND]: {
        id: exports.Stage.YOSHIS_ISLAND,
        name: "Yoshi's Island",
    },
    [exports.Stage.GREEN_GREENS]: {
        id: exports.Stage.GREEN_GREENS,
        name: "Green Greens",
    },
    [exports.Stage.FOURSIDE]: {
        id: exports.Stage.FOURSIDE,
        name: "Fourside",
    },
    [exports.Stage.MUSHROOM_KINGDOM]: {
        id: exports.Stage.MUSHROOM_KINGDOM,
        name: "Mushroom Kingdom I",
    },
    [exports.Stage.MUSHROOM_KINGDOM_2]: {
        id: exports.Stage.MUSHROOM_KINGDOM_2,
        name: "Mushroom Kingdom II",
    },
    [exports.Stage.VENOM]: {
        id: exports.Stage.VENOM,
        name: "Venom",
    },
    [exports.Stage.POKE_FLOATS]: {
        id: exports.Stage.POKE_FLOATS,
        name: "Poké Floats",
    },
    [exports.Stage.BIG_BLUE]: {
        id: exports.Stage.BIG_BLUE,
        name: "Big Blue",
    },
    [exports.Stage.ICICLE_MOUNTAIN]: {
        id: exports.Stage.ICICLE_MOUNTAIN,
        name: "Icicle Mountain",
    },
    [exports.Stage.ICETOP]: {
        id: exports.Stage.ICETOP,
        name: "Icetop",
    },
    [exports.Stage.FLAT_ZONE]: {
        id: exports.Stage.FLAT_ZONE,
        name: "Flat Zone",
    },
    [exports.Stage.DREAMLAND]: {
        id: exports.Stage.DREAMLAND,
        name: "Dream Land N64",
    },
    [exports.Stage.YOSHIS_ISLAND_N64]: {
        id: exports.Stage.YOSHIS_ISLAND_N64,
        name: "Yoshi's Island N64",
    },
    [exports.Stage.KONGO_JUNGLE_N64]: {
        id: exports.Stage.KONGO_JUNGLE_N64,
        name: "Kongo Jungle N64",
    },
    [exports.Stage.BATTLEFIELD]: {
        id: exports.Stage.BATTLEFIELD,
        name: "Battlefield",
    },
    [exports.Stage.FINAL_DESTINATION]: {
        id: exports.Stage.FINAL_DESTINATION,
...
class ConsoleConnection extends events.EventEmitter {
    ...
    _connectOnPort(ip, port, timeout) {
        ...
        const connection = reconnect({
            ...
            const handshakeMsgOut = consoleComms.genHandshakeOut(this.connDetails.gameDataCursor, this.connDetails.clientToken);
...
class DolphinConnection extends events.EventEmitter {
    ...
    async connect(ip, port) {
        ...
        this.ipAddress = ip;
        this.port = port;
        const enet = await Promise.resolve().then(function () { return _interopNamespace(require('enet')); });
...
class SlpFile extends stream.Writable {
    ...
    constructor(filePath, slpStream, opts) {
        ...
        this.slpStream = slpStream ? slpStream : new SlpStream({ mode: exports.SlpStreamMode.MANUAL });
    ...
    _onCommand(data) {
        ...
        switch (command) {
            case exports.Command.POST_FRAME_UPDATE:
                ...
                const prevPlayer = _.get(this.metadata, ["players", `${playerIndex}`]) || {};
    ...
    _setupListeners() {
        ...
        this.on("finish", () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            fs.writeSync(fd, createUInt32Buffer(this.rawDataLength), 0, "binary", 11);
            fs.closeSync(fd);
